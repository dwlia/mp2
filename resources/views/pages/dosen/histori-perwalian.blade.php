                ...
                <thead>
                  ...
                  <tbody>
                    ...
                    @foreach ($historiPerwalian as $perwalian)
                        @php
                            $sortedCatatanPerwalian = $perwalian->catatanPerwalian->sortByDesc('created_at');
                        @endphp
                        @foreach ($sortedCatatanPerwalian as $catatan)
                            <tr>
                                <td>{{ $nomorUrutan++ }}</td>
                                <td>{{ $perwalian->mahasiswa->nim }}</td>
                                <td>{{ $perwalian->mahasiswa->nama }}</td>
                                <td>{{ $perwalian->mahasiswa->program_studi }}</td>
                                <td>{{ $catatan->created_at->format('m-d-Y') }}</td>
                                <td>{{ strip_tags($catatan->catatan) }}</td>
                            </tr>
                        @endforeach
                    @endforeach
                  </tbody>
