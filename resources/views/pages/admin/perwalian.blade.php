...
<div class="container-dashboard p-5 py-5">
    ...
    <div class="row d-flex justify-content-center mt-5">
            ...
            <div class="d-flex align-items-start">
                <a href="{{ route('admin.tambahwali') }}" class="btn btn-primary mb-3 p-2" style="width: 15%"><i class="bi bi-person-fill-add"></i> Tambah Data</a>
                <form class="d-flex ms-auto" action="{{ route('admin.perwalian') }}" method="GET" role="search">
                    <input type="hidden" name="page" value="1">
                    <input class="form-control me-2" type="search" name="keyword" id="search-input" placeholder="Cari Rekap data...">
            ...
            <table class="table table-striped table-responsive mt-5 text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $perwalian->programStudi->nama_prodi }}</td>
                        <td>{{ $perwalian->dosen->nama }}</td>
                        <td>{{ $perwalian->tahunAkademik->tahun_akademik }}</td>
                        <td>{{ $perwalian->tanggal_perwalian }}</td>
                        <td>
                            <a href="{{ route('admin.editperwalian', $perwalian->id) }}" class="btn btn-warning"><i class="bi bi-pencil-square"></i>Edit</a>
                            <form action="{{ route('admin.deleteperwalian', $perwalian->id) }}" method="POST"  style="display: inline-block;">
                                @csrf
                                @method('DELETE')
